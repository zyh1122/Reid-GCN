...
#主要是：HRNet+2个dconvs（每个dconvs包含四个残差模块）
class PoseHigherResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.num_deconvs = extra.DECONV.NUM_DECONVS     #NUM_DECONVS: 1
    ...
    def _make_final_layers(self, cfg, input_channels):
        dim_tag = cfg.MODEL.NUM_JOINTS if cfg.MODEL.TAG_PER_JOINT else 1    #NUM_JOINTS: 17   TAG_PER_JOINT: True
        extra = cfg.MODEL.EXTRA
        ...
        final_layers.append(nn.Conv2d(
            ...
            kernel_size=extra.FINAL_CONV_KERNEL,    #   FINAL_CONV_KERNEL: 1
            stride=1,
    ...
    def forward(self, x):
        ...
        final_outputs = []
        x = y_list[0]                   #x[b,32,128,128]
        y = self.final_layers[0](x)     #y[b,34,128,128]
        final_outputs.append(y) #final_output=b*34*128*128

        for i in range(self.num_deconvs):   #反卷积次数为2
            if self.deconv_config.CAT_OUTPUT[i]:
                x = torch.cat((x, y), 1)

            x = self.deconv_layers[i](x)    #反卷积，把图像变大，[1,32,128,128]变为[1,32,256,256]
            y = self.final_layers[i+1](x)   #这是把list的第二个做final_layers，[b,17,256,256]
